...
$input_questions
Based on the picture and additional information, choose one of the following four choices that answers the question:
A. $choices_a
B. $choices_b
C. $choices_c
D. $choices_d
Answer the option A, B, C, D only!
""".strip())
...
def eval(args):
    ...
    for i in tqdm(range(len(json_data))):
        ...
        if vqa_acc == True:
            ...
            print(output, pred, correct_choice)
            f += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_file", type=str, default='result/final_answer_aokvqa_val_qwen.json')
    parser.add_argument("--sub_qa_path", type=str, default='result/sub_answer_aokvqa_val_qwen.json')
